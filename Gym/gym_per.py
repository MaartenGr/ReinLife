...
from Field.perdqn import DQNAgent, DQN
...
agents = [DQNAgent(151, 8) for _ in range(1)]
# models = [DQN(151, 8) for _ in range(len(agents))]
    ...
    while not all(dones):
        ...
        for i in range(len(agents)):
            if infos[i] == "Dead":
                ...
                agents[i].append_sample(s[i], actions[i], r[i], s_prime[i], dones[i])
                if agents[i].memory.tree.n_entries >= agents[i].train_start:
                    agents[i].train_model()
            elif not dones[i]:
                scores[i] += r[i]
                done_mask = 0.0 if dones[i] else 1.0
                agents[i].append_sample(s[i], actions[i], r[i], s_prime[i], dones[i])
