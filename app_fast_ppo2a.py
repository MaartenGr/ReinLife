...
agents = [PPO(101) for _ in range(1)]
...
# logging variables running_reward = 0
avg_length = 0
        ...
        for t in range(T_horizon):
            ...
            for i, agent in enumerate(agents):
                ...
                if infos[i] == "Dead" or not dones[i]:
                    ...
                    prob, action = agent.predict(torch.from_numpy(s[i].flatten()).float())
                    probs[i] = prob
                    actions[i] = action
            ...
            for i in range(len(agents)):
                if infos[i] == "Dead" or not dones[i]:
                    scores[i] += r[i]
                    agents[i].put_data(s=s[i].flatten(),
                                       action=actions[i],
                                       reward=r[i]/100.0,
                                       s_prime=s_prime[i].flatten(),
                                       prob=probs[i][actions[i]].item(),
                                       done=dones[i])
